...
char ngx_meta_lua_code_cache_key;


uintptr_t
ngx_meta_lua_escape_uri(u_char *dst, u_char *src, size_t size, ngx_uint_t type)
{
    ngx_uint_t      n;
    uint32_t       *escape;
...
void
ngx_meta_lua_unescape_uri(u_char **dst, u_char **src, size_t size,
    ngx_uint_t type)
{
...
u_char *
ngx_meta_lua_digest_hex(u_char *dest, const u_char *buf, int buf_len)
{
    ngx_md5_t                     md5;
...
void
ngx_meta_lua_set_multi_value_table(lua_State *L, int index)
{
    if (index < 0) {
