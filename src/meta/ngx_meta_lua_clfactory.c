...
#include <nginx.h>
#include "ngx_meta_lua_clfactory.h"
...
typedef enum {
    ...
    NGX_LUA_BT_LUA,
    NGX_LUA_BT_LJ
} ngx_meta_lua_clfactory_file_type_e;
...
typedef struct {
    ngx_meta_lua_clfactory_file_type_e file_type;
...
} ngx_meta_lua_clfactory_file_ctx_t;
...
typedef struct {
    ...
    const char *s;
    size_t      size;
} ngx_meta_lua_clfactory_buffer_ctx_t;


static const char *ngx_meta_lua_clfactory_getF(lua_State *L, void *ud,
    size_t *size);
static int ngx_meta_lua_clfactory_errfile(lua_State *L, const char *what,
    int fname_index);
static const char *ngx_meta_lua_clfactory_getS(lua_State *L, void *ud,
    size_t *size);
static long ngx_meta_lua_clfactory_file_size(FILE *f);


int
ngx_meta_lua_clfactory_bytecode_prepare(lua_State *L,
    ngx_meta_lua_clfactory_file_ctx_t *lf, int fname_index)
{
    ...
    *lf->begin_code.str = LUA_SIGNATURE[0];
    ...
    if (lf->file_type == NGX_LUA_BT_LJ) {
        ...
        fsize = ngx_meta_lua_clfactory_file_size(lf->f);
        if (fsize < 0) {
            serr = strerror(errno);
...
ngx_int_t
ngx_meta_lua_clfactory_loadfile(lua_State *L, const char *filename)
{
    int                         c, status, readstatus;
    ngx_flag_t                  sharp;

    ngx_meta_lua_clfactory_file_ctx_t        lf;
    ...
    if (lf.f == NULL) {
        return ngx_meta_lua_clfactory_errfile(L, "open", fname_index);
    }
    ...
    if (c == LUA_SIGNATURE[0] && filename) {  /* binary file? */
        ...
        if (lf.f == NULL) {
            return ngx_meta_lua_clfactory_errfile(L, "reopen", fname_index);
        }
        ...
        status = ngx_meta_lua_clfactory_bytecode_prepare(L, &lf, fname_index);
    ...
    status = lua_load(L, ngx_meta_lua_clfactory_getF, &lf,
                      lua_tostring(L, -1));
    ...
    if (readstatus) {
        ...
        return ngx_meta_lua_clfactory_errfile(L, "read", fname_index);
    }
...
ngx_int_t
ngx_meta_lua_clfactory_loadbuffer(lua_State *L, const char *buff,
    size_t size, const char *name)
{
    ngx_meta_lua_clfactory_buffer_ctx_t     ls;
    ...
    return lua_load(L, ngx_meta_lua_clfactory_getS, &ls, name);
}


static const char *
ngx_meta_lua_clfactory_getF(lua_State *L, void *ud, size_t *size)
{
    char                        *buf;
    size_t                       num;
    ...
    lf = (ngx_meta_lua_clfactory_file_ctx_t *) ud;
...
static int
ngx_meta_lua_clfactory_errfile(lua_State *L, const char *what, int fname_index)
{
    const char      *serr;
    const char      *filename;
...
static const char *
ngx_meta_lua_clfactory_getS(lua_State *L, void *ud, size_t *size)
{
    ngx_meta_lua_clfactory_buffer_ctx_t      *ls = ud;
...
static long
ngx_meta_lua_clfactory_file_size(FILE *f)
{
    long              cur_pos, len;
