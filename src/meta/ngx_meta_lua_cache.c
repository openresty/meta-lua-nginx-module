 ...
 * */
static ngx_int_t
ngx_meta_lua_cache_load_code(ngx_log_t *log, lua_State *L,
    const char *key)
{
    int          rc;
    u_char      *err;
    ...
    lua_pushlightuserdata(L, &ngx_meta_lua_code_cache_key);
 ...
 * */
static ngx_int_t
ngx_meta_lua_cache_store_code(lua_State *L, const char *key)
{
    int rc;
...
ngx_int_t
ngx_meta_lua_cache_loadbuffer(ngx_log_t *log, lua_State *L,
    ...
    const char *name)
{
    ...
    rc = ngx_meta_lua_cache_load_code(log, L, (char *) cache_key);
    if (rc == NGX_OK) {
    ...
    rc = ngx_meta_lua_clfactory_loadbuffer(L, (char *) src, src_len, name);
     ...
     *  code cache */
    rc = ngx_meta_lua_cache_store_code(L, (char *) cache_key);
    if (rc != NGX_OK) {
...
ngx_int_t
ngx_meta_lua_cache_loadfile(ngx_log_t *log, lua_State *L,
    const u_char *script, const u_char *cache_key)
{
    int              n;
    ngx_int_t        rc, errcode = NGX_ERROR;
    u_char          *p;
    u_char           buf[NGX_META_LUA_FILE_KEY_LEN + 1];
    const char      *err = NULL;
    ...
    if (cache_key == NULL) {
        ...
        p = ngx_copy(buf, NGX_META_LUA_FILE_TAG, NGX_META_LUA_FILE_TAG_LEN);

        p = ngx_meta_lua_digest_hex(p, script, ngx_strlen(script));
    ...
    rc = ngx_meta_lua_clfactory_loadfile(L, (char *) script);
